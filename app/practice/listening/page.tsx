...
export default function ListeningPracticePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-6"><div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-2"><span className="text-3xl">🎧</span><h1 className="text-3xl font-bold text-gray-900">Listening Practice</h1></div>
      <p className="text-gray-500 mb-8">Listen to audio passages and answer comprehension questions.</p>
      ...
      {loading ? <div className="flex items-center justify-center py-20"><div className="w-10 h-10 border-4 border-orange-600 border-t-transparent rounded-full animate-spin" /></div>
      ...
      : <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map(task => (
            <Link key={task.id} href={`/listening?taskId=${task.id}`} className="group bg-white border border-orange-100 rounded-xl p-6 hover:shadow-md transition-all hover:-translate-y-0.5">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-semibold text-orange-600">Start Practice →</span>
              </div>
            </Link>
          ))}
        </div>}
