  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-6">
      <div className="max-w-5xl mx-auto">
        <PracticeBackLink fallback="/practice" className="text-sm text-purple-600 hover:underline mb-4 block">
          ← Back
        </PracticeBackLink>
        ...
        <p className="text-gray-600 mb-8">Practice all 8 speaking tasks with timers and AI feedback.</p>
        <div className="flex gap-2 mb-6 flex-wrap">
          <button
            type="button"
            onClick={() => setFilter("all")}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
              filter === "all" ? "bg-purple-600 text-white" : "bg-white text-gray-600 border border-gray-200 hover:bg-gray-50"
            }`}
          >
            All
          </button>
          {TYPES.map(t => (
            <button
              ...
              onClick={() => setFilter(t)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                filter === t ? "bg-purple-600 text-white" : "bg-white text-gray-600 border border-gray-200 hover:bg-gray-50"
              }`}
            >
              {t.replace("Speaking ", "")}
            </button>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map(task => (
              <Link
                key={task.id}
                href={buildTaskPracticeHref("speaking", task.id, listPath)}
                className="group bg-white border border-purple-100 rounded-xl p-6 hover:shadow-md transition-all hover:-translate-y-0.5"
              >
                <span className="text-xs font-bold px-2 py-1 rounded-full bg-purple-100 text-purple-700">{task.task_type}</span>
                <h3 className="font-bold text-gray-800 mt-3 mb-2 group-hover:text-purple-600 transition">
                  {task.content?.situation?.slice(0, 80) || task.title}
                </h3>
                <p className="text-xs text-gray-600 mb-4">{INFO[task.task_type]?.desc}</p>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-600">{INFO[task.task_type]?.time}</span>
                  <span className="text-xs font-semibold text-purple-600 group-hover:underline">Start Practice →</span>
                </div>
              </Link>
            ))}
          </div>
