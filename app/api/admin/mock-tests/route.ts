...
export async function GET(request: Request) {
  ...
  const { data: assigned } = await admin.from("mock_test_tasks").select("task_id");
...
export async function POST(request: Request) {
  ...
  if (insertErr || !inserted) {
    return NextResponse.json({ error: insertErr?.message || "Failed to create mock test." }, { status: 500 });
  }
  ...
  if (linkErr) {
    await admin.from("mock_tests").delete().eq("id", inserted.id);
    const msg =
      linkErr.code === "23505"
